...
impl Server {
	...
	pub async fn listen(self) -> io::Result<()> {
		...
		println!("Server started on port 4221!");
			...
			match listener.accept().await {
				Ok((socket, _)) => {
					...
					tokio::spawn(async move {
						let inner_handler = handler.lock().await;
						inner_handler.clone().handle_socket(socket).await;
					});
				}
				Err(e) => {
	...
	pub async fn register_endpoint(&mut self, verb: HttpVerb, path: String, handler: fn(Request) -> String) {
		...
		registry.endpoints.insert(endpoint_key, Box::new(handler));
	}

	pub async fn get(&mut self, path: String, handler: fn(Request) -> String) {
		self.register_endpoint(HttpVerb::GET, path, handler).await;
	}

	pub async fn post(&mut self, path: String, handler: fn(Request) -> String) {
		self.register_endpoint(HttpVerb::POST, path, handler).await;
	}

	pub async fn put(&mut self, path: String, handler: fn(Request) -> String) {
		self.register_endpoint(HttpVerb::PUT, path, handler).await;
	}

	pub async fn delete(&mut self, path: String, handler: fn(Request) -> String) {
		self.register_endpoint(HttpVerb::DELETE, path, handler).await;
	}

	pub async fn head(&mut self, path: String, handler: fn(Request) -> String) {
		self.register_endpoint(HttpVerb::HEAD, path, handler).await;
	}

	pub async fn options(&mut self, path: String, handler: fn(Request) -> String) {
		self.register_endpoint(HttpVerb::OPTIONS, path, handler).await;
	}

	pub async fn trace(&mut self, path: String, handler: fn(Request) -> String) {
		self.register_endpoint(HttpVerb::TRACE, path, handler).await;
	}
	
	/// Serves a directory of static files at the given endpoint.
	/// leave the endpoint empty to serve the directory at the root.
	pub async fn serve(&mut self, path: String, directory: String) {
		...
		let mut registry = self.registry.lock().await;
		registry.static_directories.insert(normalized_path, directory);
	}

	pub fn respond(status: Option<u16>, body: Option<String>, headers: Option<HashMap<String, String>>) -> String {
		...
		if !body_string.is_empty() {
			...
			header_map.entry(String::from("Content-Type")).or_insert(String::from("text/plain"));
			header_map.entry(String::from("Content-Length")).or_insert(body_string.len().to_string());
		}
		...
		for (path, dir) in self.static_directories.iter() {
			...
			match file_contents {
				Ok(contents) => {
					...
					return Server::respond(Some(200), Some(contents), Some(
						[
